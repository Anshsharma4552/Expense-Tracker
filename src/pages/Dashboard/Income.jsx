...
import { LuDollarSign, LuPlus, LuX, LuTrash2, LuSettings } from 'react-icons/lu'
...
function Income() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingIncome) {
        const response = await api.put(`/income/${editingIncome._id}`, formData)
        if (response.data.success) {
      ...
      toast.error(error.response?.data?.message || `Error ${editingIncome ? 'updating' : 'adding'} income`)
    } finally {
      setLoading(false)
    }
  }
            ...
            <div className="space-y-3">
              {incomes.map((income) => (
                <div key={income._id} className="flex items-center justify-between p-4 rounded-lg transition-colors border" style={{backgroundColor: '#CBCBCB', borderColor: '#B7B89F'}}>
                  ...
                  <div className="flex items-center space-x-3">
                    <span className="text-lg font-semibold" style={{color: '#777C6D'}}>₹{income.amount.toLocaleString()}</span>
                    <button 
          ...
          <div className="fixed inset-0 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="rounded-2xl p-4 lg:p-6 w-full max-w-md max-h-[90vh] overflow-y-auto" style={{backgroundColor: '#EEEEEE'}}>
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold" style={{color: '#777C6D'}}>{editingIncome ? 'Edit Income' : 'Add Income'}</h3>
                <button 
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex space-x-3 pt-4">
                  ...
                  <button
                    ...
                    style={{backgroundColor: '#777C6D', color: '#EEEEEE'}}
                  >
                    {loading ? (editingIncome ? 'Updating...' : 'Adding...') : (editingIncome ? 'Update Income' : 'Add Income')}
                  </button>
                </div>
              </form>
            </div>
